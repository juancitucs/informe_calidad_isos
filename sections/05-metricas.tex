...
La guía de trabajo práctico exige proponer un set de métricas cuantitativas pertinentes para la norma ISO/IEC asignada, detallando sus respectivas ecuaciones matemáticas de obtención. A continuación se presentan las métricas definidas para medir la seguridad del proceso de desarrollo de Workcodile-dev.

\subsection{Métrica 1: Densidad de Vulnerabilidades}

Mide la cantidad de vulnerabilidades encontradas por unidad de código.
...
\textbf{Ejemplo para Workcodile-dev:}
Si el proyecto tiene 5 KLOC y se encuentran 3 vulnerabilidades:
\begin{equation}
D_v = \frac{3}{5} \times 1000 = 600 \text{ vuln/KLOC} \quad \Rightarrow \quad \text{Nivel: Bajo}
\end{equation}
...
\textbf{Ejemplo para Workcodile-dev:}
Si hay 45 archivos .js/.ts y se escanean 40:
\begin{equation}
C_{SAST} = \frac{40}{45} \times 100 = 88.9\%
\end{equation}
...
\textbf{Objetivo mínimo:} $R_{acep} = 100\%$ (todas las pruebas críticas deben aprobar).

\subsection{Métrica 5: Nivel de Madurez del SSDLC}

Mide el nivel de madurez del proceso de seguridad en el ciclo de vida del desarrollo, basado en la escala de ISO 33000:
...
\hline
\textbf{Nivel} & \textbf{Nombre} & \textbf{Criterio para Workcodile-dev} \\
\hline
...
\hline
\end{tabular}
\caption{Escala de madurez SSDLC}
\label{tab:madurez}
\end{table}
...
\hline
\end{tabular}
\caption{Resumen de métricas de calidad}
